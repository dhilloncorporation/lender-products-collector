...
class BingSearchEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        """Initialize Bing search engine with configuration."""
        self.config = config
        self.name = config.get('name', 'Bing')
        self.base_url = config.get('base_url', 'https://www.bing.com/search')
        self.params = config.get('params', 'q={query}&count={num}&mkt={mkt}')
        ...
        self.rate_limit_per_hour = config.get('rate_limit_per_hour', 150)
    
    def build_search_url(self, query: str, max_results: int = 5) -> str:
        """Build Bing search URL."""
        encoded_query = quote_plus(query)
        return f"{self.base_url}?q={encoded_query}&count={max_results}&mkt=en-AU"
    ...
    async def extract_results(self, page) -> List[str]:
        ...
        logger.debug(f"Found {len(result_links)} total links on Bing page")
        ...
        for link in result_links:
            ...
            # Bing uses direct links in most cases
            if href.startswith('http') and 'bing.com' not in href and 'microsoft.com' not in href:
                urls.append(href)
                logger.debug(f"Extracted Bing URL: {href}")
